...
from constants.ultrasound import IMAGE_TYPE, READOUT_ABBREVS as RA, WALL_FILTER_MODES
...
def isolate_text(grayscale_image, image_type):
    ...
    left_bar_crop = grayscale_image[120:200, :120].copy()
    scale_crop = grayscale_image[15:40, 585:].copy()

    left_bar_crop = cv2.resize(
        left_bar_crop,
        None,
        fx=4, 
        fy=4)
    ...
    raw_text = pytesseract.image_to_string(
        left_bar_crop, lang="eng", boxes=False, config=r"--psm 11 -c tessedit_char_whitelist=0123456789.W",
    )

    # Specifically whitelist numerical characters and "." to aid the OCR engine
    scale_text_raw = pytesseract.image_to_string(
        scale_crop,
        lang="eng",
        boxes=False,
    ...
    text_segments = [
        segment.upper().strip() for segment in text_segments if segment is not ""
    ]

    # Isolate the frame scale (e.g. 2.4 cm, 3.9 cm)
    try:
        scale_segments = scale_text_raw.splitlines()
        scale_segments = [
            segment.upper().strip() for segment in scale_segments if segment is not ""
        ]
        scale_segments = [
            max(re.findall(r"[\d]+(?:\.[\d]+)*", segment), key=len)
    ...
    if image_type is IMAGE_TYPE.COLOR:
        ...
        CPA_check = (
            len([segment for segment in text_segments if RA.CPA in segment]) == 1
        )
        COL_check = (
            len([segment for segment in text_segments if RA.COLOR_LEVEL in segment])
            == 1
        )
        ...
        mode_segment = [segment for segment in text_segments if found_mode in segment][
            0
        ]
        ...
        numerical_strings = [
            num_string for num_string in re.findall(r"[\d]+(?:\.[\d]+)*", mode_segment)
        ]
        ...
        WF_check = (
            len([segment for segment in text_segments if RA.WALL_FILTER in segment])
            == 1
        )

        if not WF_check:
            raise IOError(
                "Wall filter check did not pass for image. Incorrect matching segments"
            )

        wf_segment = [
            segment for segment in text_segments if RA.WALL_FILTER in segment
        ][0]
        ...
        if wf_segment not in WALL_FILTER_MODES:
            ...
            raise IOError(
                "Wall filter check did not pass for image. Incorrect mode: ${0}".format(
                    wf_segment
                )
            )
        ...
        PRF_check = (
            len(
                [
                    ...
                    if RA.PULSE_REPITITION_FREQUENCY in segment
                ]
            )
            == 1
        )
        if not PRF_check:
            raise IOError(
                "PRF check did not pass for image. Incorrect matching segments"
            )
        ...
        numerical_strings = [
            num_string for num_string in re.findall(r"[\d]+(?:\.[\d]+)*", prf_segment)
        ]
        numerical_strings = [ns.replace(".", "") for ns in numerical_strings]
